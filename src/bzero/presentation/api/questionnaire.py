...
async def get_questionnaires(
    ...
    results, total = await GetQuestionnairesUseCase(questionnaire_service).execute(
        user_id=user_id,
        offset=offset,
        limit=limit,
    )
    ...
    return QuestionnaireListResponse(
        questionnaires=[QuestionnaireResponse.create_from(result) for result in results],
        total=total,
        offset=offset,
        limit=limit,
    )
...
async def get_questionnaire_by_id(
    ...
    result = await GetQuestionnaireByIdUseCase(questionnaire_service).execute(
